...
- **Modify:** `TransactionActor.add`, `.addBatch`, `.update`, `.delete`, `.addRecurrenceRule`, `.updateRecurrenceRule`, `.deleteOccurrences` — after the mutation commits, call `SafeToSpendSnapshotWriter.write()` (which itself invokes `SpendingForecastService` to get a current projection) and `WidgetCenter.shared.reloadTimelines`. These are the true choke points every transaction-affecting change passes through; `saveForecastCache` is not one of them, since its only caller is `InsightsViewModel.computeForecast()`.
