...
private fun CategoryEditorSheet(
    ...
    val nameError = categoryNameError(trimmedName, type, category?.id, existingCategories)
...
private fun categoryNameError(
    ...
    name.isEmpty() -> CategoryNameError.BLANK
...
private fun categoryIcon(token: String): ImageVector = when {
    ...
    token.contains("fork") || token.contains("cup") -> Icons.Outlined.Restaurant
    ...
    token.contains("car") || token.contains("fuel") -> Icons.Outlined.DirectionsCar
    token.contains("star") || token.contains("trophy") -> Icons.Outlined.Star
