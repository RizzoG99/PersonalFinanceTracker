...
- Travels: transactions already in a trip now say which trip, on the line under the transaction name: category, then the trip's own icon and name ("Intrattenimento · ⛱ Madrid"). The icon used to sit next to the transaction name, where on a transaction you hadn't renamed it looked like part of the category. This shows while you're multi-selecting, in search results, in filtered lists, and in the Dashboard's recent transactions — anywhere a trip expense sits next to one that isn't in a trip.
- VoiceOver: while multi-selecting in Activity, rows now say whether they're selected — previously a selected row sounded exactly like an unselected one, with Delete one tap away. Repeating transactions also now read as "Recurring" instead of the icon's own name.
- Travels: the Travels list (⋯ menu in Activity) now opens only as tall as the trips in it, instead of covering the screen to show one trip. Drag it up for the full height as before.
- Travels: the expenses inside a trip are no longer squashed together — they're spaced like the Dashboard's "Recent transactions" list.
- **The daily check-in card no longer offers to log your subscriptions and instalments.** The Financial Pulse card was suggesting things like "Log iCloud −9,99 €" or "Log Finanziamento Moto −260,00 €" — but the app already records every recurring entry by itself, so tapping one filed it a second time. Those are now left out entirely, and the card suggests only the everyday spends you enter by hand: ones you have logged on three or more separate days in the last two months, and at least once in the last fortnight. If nothing you buy is that regular yet, the card simply shows "Add Transaction" and "Confirm no spending" as before.
